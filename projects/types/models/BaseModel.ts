...
export abstract class BaseModel {
  ...
  protected _formGroup: UntypedFormGroup | any;

  addedFormControls: any;
  // eslint-disable-next-line @typescript-eslint/naming-convention, no-underscore-dangle, id-blacklist, id-match
  private _addedFormControls: any = {};
  private static clearFormArray(formGroup: UntypedFormGroup, key: string): any {
    if (formGroup) {
  ...
  protected setValuesInAddedPropertiesOfAttachedFormControls(
    ...
    const rawValues: any = this.getValuesToUse(values, useFormGroupValuesToModel);
    let key: any;
    for (key in this.addedFormControls) {
      if (rawValues[key]) {
        if (useFormGroupValuesToModel) {
          ...
          if (control instanceof UntypedFormControl) {
            ...
            key.setValues(rawValues[key], useFormGroupValuesToModel);
  ...
  protected setFormGroupValuesInAddedFormControls(): void {
    ...
    let key: any;
    for (key in this.addedFormControls) {
      const control: any = this.addedFormControls[key];
      if (control instanceof UntypedFormControl) {
        (control as UntypedFormControl).setValue(key);
      } else if (control instanceof UntypedFormGroup) {
        key.setFormGroupValues();
      } else if (control instanceof UntypedFormArray) {
        throw new Error(`control of type FormArray not supported here`);
      } else {
        throw new Error(`control of type ${control} not supported here`);
